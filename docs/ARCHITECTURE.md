...
3. Host address or alias plus a unique published port → `medium` confidence. A view host also stands for the one machine another connector reports. A tailnet device (kind `tailscale`) and a Proxmox VM, container or node (kinds `vm`, `lxc`, `proxmox-node`) link by one of their addresses or names, else by a unique short host name. A DNS view (kind `dns`: the names a local resolver such as Pi-hole answers for one IP) links only by that IP, never by a name, and never for a loopback or unspecified record. Two candidate machines, or two views of the same family claiming one machine, leave the view unlinked; one machine can have a tailnet device, a DNS view and a Proxmox guest at once. A view never scopes a proxy by itself, since it runs no services. A loopback upstream also matches a loopback-only listener on the proxy's own host.
