...
- Traefik and Caddy connectors issue GET requests. Nginx Proxy Manager uses its token-authentication POST, then GETs proxy hosts and certificates; it does not edit NPM configuration. Traefik GETs that carry configured basic auth or a header, and every NPM GET (which carries its token), never follow redirects.
