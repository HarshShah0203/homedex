...
- Traefik and Caddy connectors issue GET requests. Nginx Proxy Manager uses its token-authentication POST, then GETs proxy hosts and certificates; it does not edit NPM configuration. The NPM token POST, which carries the NPM password, never follows a redirect. Traefik GETs that carry configured basic auth or a header, and every NPM GET (which carries its token), follow a redirect only to the same host name and never from HTTPS to plain HTTP.
...
- The Proxmox VE connector authenticates with an API token and only GETs the token's own effective permissions, `cluster/resources`, `cluster/status` and, for running guests, the QEMU guest agent's `network-get-interfaces` or the container interface list. It never reads guest configs (descriptions, SSH keys, cloud-init, container environment variables), consoles, the HMP monitor, any other guest agent command, storage, users or ACLs, and a build tripwire fails if it ever names one. It verifies the node's certificate against a pinned fingerprint, a pinned CA or the system roots, never skips verification, and does not follow redirects. Proxmox's own error text is never stored. PVEAuditor on PVE 9 could also read guest configs and run the monitor's `info` commands; the connector's allow-list, not the role, keeps those out, so [the connector guide](docs/CONNECTORS.md#proxmox-ve) also gives a tighter custom role.
