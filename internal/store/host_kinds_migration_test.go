...
// TestNewHostKindsMigrationKeepsEveryHostColumn upgrades a populated v0010
// database. hosts is rebuilt to widen its kind CHECK, so ids, aliases,
// reported_last_seen and power_state must survive, every row referencing a host by id must
...
func TestNewHostKindsMigrationKeepsEveryHostColumn(t *testing.T) {
	...
	for _, statement := range []string{
		...
		`INSERT INTO hosts(id,connector_id,natural_key,name,kind,address,parent_host_id,state,power_state,first_seen,last_seen,created_at,updated_at) VALUES(102,3,'ssh:vm','vm','ssh','10.0.0.3',101,'gone','stopped','` + now + `','` + now + `','` + now + `','` + now + `')`,
	...
	var power string
	if err = q.QueryRow(`SELECT power_state FROM hosts WHERE id=102`).Scan(&power); err != nil || power != "stopped" {
		t.Fatalf("power_state after migration = %q, %v, want stopped", power, err)
	}
